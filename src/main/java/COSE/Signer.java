...
public class Signer extends Attribute {
    ...
    public void setKey(CBORObject cnKeyIn) throws CoseException {
        cnKey = new OneKey(cnKeyIn);
    }
    ...
    static boolean validateSignature(AlgorithmID alg, byte[] rgbToBeSigned, byte[] rgbSignature, OneKey cnKey) throws CoseException {
        ...
        switch (alg) {
            ...
            case ECDSA_512:
            {
                ...
                X9ECParameters p = cnKey.GetCurve();
                ECDomainParameters parameters = new ECDomainParameters(p.getCurve(), p.getG(), p.getN(), p.getH());
                BigInteger bnX = new BigInteger(1, cnKey.getValue(KeyKeys.EC2_X.AsCBOR()).GetByteString());
                ECPoint point = p.getCurve().createPoint(bnX, new BigInteger(1, cnKey.getValue(KeyKeys.EC2_Y.AsCBOR()).GetByteString()));
